...
func prepareInsertStmt(tx *sql.Tx) (*sql.Stmt, error) {
	return tx.Prepare(`
		INSERT INTO Product (
			...
			proCategoryName, proCategoryParentCode, proCreateDate, proCreateName
		) VALUES (
...
func execInsert(stmt *sql.Stmt, rec Product) error {
	...
	_, err = stmt.Exec(
		rec.ProCode,                    // @p1  → ProductId = proCode
...
func execUpdate(stmt *sql.Stmt, rec Product) error {
	...
	_, err = stmt.Exec(
		rec.ProCode,                    // @p1  → productId = proCode (cập nhật lại)
		rec.ProName,                    // @p2
		rec.ProType,                    // @p3
		rec.ProStatus,                  // @p4
		rec.ProIsSale,                  // @p5
...
func SaveBatch(records []Product) error {
	...
	for _, rec := range records {
		// Gán ProductId = ProCode (dự phòng)
		rec.ProductId = rec.ProCode
